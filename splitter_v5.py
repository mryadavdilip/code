...
def parse_font_arg(font_arg):
    params = {}
    if font_arg:
        parts = font_arg.strip().split(" -")
        for part in parts:
            if part:
...
def create_thumbnail(text, output_path, size=(640, 360), font_args=None):
    img = Image.new("RGB", size, font_args.get("bg_color", "#000000"))
    draw = ImageDraw.Draw(img)

    font_family = font_args.get("family", "arial.ttf")
    font_size = int(font_args.get("size", 40))
    color = font_args.get("color", "#FFFFFF")
...
def generate_letterbox_text_filter(font_args, text, position, video_width, video_height):
    if not text:
        return ""

    visibility = font_args.get("visibility", "5s")
    height = float(font_args.get("height", "5"))
    margin_left = int(font_args.get("margin_left", "20"))
    margin_right = int(font_args.get("margin_right", "20"))
    text_align = font_args.get("text_align", "center")
    font_size = int(font_args.get("size", "20"))
    color = font_args.get("color", "white")
    fontfile = font_args.get("family", "arial.ttf")

    y_expr = {
        "top": f"(h*{height}/100)-(text_h/2)",
        "bottom": f"(h - (h*{height}/100))-(text_h/2)"
    }[position]

    align_expr = {
        "center": f"(w-text_w)/2",
        "left": f"{margin_left}",
        "right": f"(w-text_w-{margin_right})"
    }.get(text_align, "(w-text_w)/2")

    return (
        f"drawtext=text='{text}':"
        f"enable='lt(t,{visibility[:-1]})':"
        f"x={align_expr}:y={y_expr}:"
...
def split_video_fast(args):
    ...
    top_font_args = parse_font_arg(args.letterbox_top_font)
    bottom_font_args = parse_font_arg(args.letterbox_bottom_font)
    thumb_font_args = parse_font_arg(args.thumbnail_font)
    ...
    while start < duration:
        ...
        letterbox_texts = (args.letterbox_setting or "").split(" -")
        letterbox_top_text = ""
        letterbox_bottom_text = ""

        for section in letterbox_texts:
            if section.startswith("top-"):
                letterbox_top_text = section[4:].replace("$input", os.path.basename(args.input)).replace("$part", str(part_num))
            if section.startswith("bottom-"):
                letterbox_bottom_text = section[7:].replace("$input", os.path.basename(args.input)).replace("$part", str(part_num))

        text_filters = []
        if letterbox_top_text:
            text_filters.append(generate_letterbox_text_filter(top_font_args, letterbox_top_text, "top", video_width, video_height))
        if letterbox_bottom_text:
            text_filters.append(generate_letterbox_text_filter(bottom_font_args, letterbox_bottom_text, "bottom", video_width, video_height))

        filter_complex = ",".join(filter(None, ["transpose=1"] + text_filters))
        ...
        split_cmd = [
            ...
            '-vf', filter_complex,
            '-c:a', 'copy', video_file, '-y'
        ]
        subprocess.run(split_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

        create_thumbnail(video_name, thumb_path, size=(video_width, video_height), font_args=thumb_font_args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--letterbox_setting', help='Letterbox configuration, e.g. -top- $input -bottom- $part')
    parser.add_argument('--letterbox_top_font', help='Font settings for top letterbox text')
    parser.add_argument('--letterbox_bottom_font', help='Font settings for bottom letterbox text')
    parser.add_argument('--thumbnail_font', help='Font settings for thumbnails')
